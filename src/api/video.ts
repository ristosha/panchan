...
export async function getVideoMetadata (inputFile: string): Promise<VideoMetadata> {
  const options: string = '-v error -show_entries format=duration:stream=width,height,r_frame_rate,codec_type -of json'
  ...
  const { width, height, r_frame_rate } = streams.find((str: { codec_type: string }) => str.codec_type === 'video')

  const [fpsNumerator, fpsDenominator] = r_frame_rate.split('/').map(Number)
  const fps = Math.floor(fpsNumerator / fpsDenominator)

  return { duration, width, height, fps, hasAudio }
}
